...
int lparse (int argc, char *argv[], char *output_file)
{
	...
	for (int i = 0; i<argc; i++) {
		cout <<"Par "<<argv[i] << endl;
	}
	RestrictType res = RT_STRONG;
	initialize_static();
	cout << "process arguments " << endl;
	process_arguments(argc, argv);
	cout << "process arguments after" << endl;

	register_functions();

	if (sys_data.print_registered) {
		function_table->PrintRegistered();
		// freopen("prova","w",stdout);
		return(1);
		//exit (1);
	}

	FILE *stream;
	char *finalname = output_file;
	//cout <<"Name output of lparse is "<< finalname << endl;
	fgetpos(stdout, &pos);
	//cout <<"After fgetpos" << endl;
	fd = dup(fileno(stdout));
	//cout <<"After dup" << endl;
	 if((stream = freopen(finalname, "w", stdout)) == NULL) {
	     cout << "Impossible to freopen" << finalname << endl;
		 exit(-1);
	 }
	 //cout << "before yyaprse" << endl;
#ifndef CXX_HAS_NO_EXCEPTIONS
	try {
#endif
		//cout <<"befire yyparse" << endl;
		yyparse();
	...
	fflush(stdout);
		...
		clearerr(stdout);
		fsetpos(stdout, &pos);
		//cout << "Closing fucking stream " << stream << endl;
		//fclose(stream);
	delete_vars();
